...
from integrations.telegram_config import TELEGRAM_BOT_TOKEN, TELEGRAM_BOT_HANDLE
...
async def initiate_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Greetings! I am your bot. How can I assist you today?"
    )
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type == "group":
        if TELEGRAM_BOT_HANDLE in text:
            cleaned_text: str = text.replace(TELEGRAM_BOT_HANDLE, "").strip()
